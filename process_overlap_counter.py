...
parser.add_argument('--overlaps', required=True,
                    help='Path to Overlaps.txt.')
parser.add_argument('--overlap-summary', required=True,
                    help='Path to OverlapSummary.txt.')
parser.add_argument('--output-dir', required=True,
                    help='Path to an existing output directory to write output files.')
parser.add_argument('--rfilter', dest='rfilter',
                    help='Filepath of regions (e.g., known repetitive regions) to exclude.')
...
# Step 1: Load data & optionally filter repetitive regions.
data = pd.read_csv(overlaps, sep="\t").values
summarydata = pd.read_csv(overlap_summary, sep="\t").values
...
# Step 2: Filter repetitive elements.
...
column_names = [rcelliddict[i] for i in range(matrix.shape[1])]
